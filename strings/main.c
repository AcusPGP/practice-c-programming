...
int main() {
     ...
     */
     ...
     // value = strcspn(string1, "evo"); // Determines and returns the length of the initial segment of string s1 consisting of characters not contained in string s2. --> value = 1
     // value = strspn(string1, "olle"); // The length of the initial portion of str1 containing only characters that appear in str2.
     ...
     // result = strrchr(string1, 'l'); // Locates the last occurrence of c in string s. 
                                        // If c is found, a pointer to c in string s is returned. Otherwise, a NULL pointer is returned. --> result = "llo"
     // result = strstr(string1, "ello"); // Locates the first occurrence in string s1 of string s2. 
     ...
     */
     ...
     // // result = memset(string1, 'x', 3); // Copies c (converted to unsigned char) into the first n characters of the object pointed to by s. A pointer to the result is returned. --> result = "xxxlo"
     ...
     */
     // char *result;
     // int value;
     // result = strerror(4); // Creates a system-dependent error message based on errornum. Returns a pointer to the string
